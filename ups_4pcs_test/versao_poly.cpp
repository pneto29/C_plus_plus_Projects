...
int main (int argc, char** argv)
{
 ...
 //   int lf = atof(argv[7]);
    ...
    float it = atof(argv[7]);

    //inicialization
 /* pcl::registration::FPCSInitialAlignment<pcl::PointXYZ, pcl::PointXYZ> sfpcs;
    sfpcs.setInputSource(source);  //
    sfpcs.setInputTarget(target);  //
    sfpcs.setApproxOverlap(it);// Defina a sobreposição aproximada entre a origem e o destino.
    //  sfpcs.setLambda(0.5);
...
*/
            ...
            for (int p = 0; p < 3; p++) {
                ...
                if (log(eigentropy) < 0.47 && log(omnvariance) < 0.47) {
    		     ...
    		     pcl::registration::FPCSInitialAlignment<pcl::PointXYZ, pcl::PointXYZ> sfpcs;
                    ...
                    case X:
                        ...
                        sfpcs.setInputSource(partitionsx_s[i]);
                        sfpcs.setInputTarget(partitionsx_t[i]);

                        break;
                    case Y:

                        sfpcs.setInputSource(partitionsy_s[i]);
                        sfpcs.setInputTarget(partitionsy_t[i]);

                        break;
                    case Z:

                        sfpcs.setInputSource(partitionsz_s[i]);
                        sfpcs.setInputTarget(partitionsz_t[i]);
   		     ...
   		     sfpcs.setApproxOverlap(it);// Defina a sobreposição aproximada entre a origem e o destino.
        	     sfpcs.setNumberOfSamples(1000); //Defina o número de amostras de nuvem de ponto de origem a serem usadas durante o registro
        	     //sfpcs.setDelta(0.5);
		     sfpcs.align(*aux_cloud);
		     pcl::IterativeClosestPoint<pcl::PointXYZ, pcl::PointXYZ> icp;
		     icp.setInputSource(aux_cloud);
		     icp.setInputTarget(target);
		     icp.setMaximumIterations(atoi(argv[5]));
                    icp.align(*aux_cloud);		     

                    // Transform source cloud
                    Eigen::Matrix4f rt = sfpcs.getFinalTransformation();
